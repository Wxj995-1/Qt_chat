...
bool CAsyncLog::m_bConsoleOutput = true;
...
bool CAsyncLog::init(const char *pszLogFileName /* = nullptr*/, bool bTruncateLongLine /* = false*/, int64_t nRollSize /* = 10 * 1024 * 1024*/)
{
    ...
    if (!m_strFileName.empty())
    {
        ...
        if (pLastSlash != nullptr)
            *pLastSlash = '\0';
        strLogDir = std::string(szPath) + "\\log";
        _mkdir(strLogDir.c_str());
        m_strFileName = strLogDir + "\\" + m_strFileName;
#else
        if (m_strFileName[0] != '/')
        {
            char szPath[1024];
...
void CAsyncLog::setConsoleOutput(bool bOutput)
{
    m_bConsoleOutput = bOutput;
}
